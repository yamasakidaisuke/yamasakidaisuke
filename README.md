...
**事業実装アーキテクト** — まだ言葉になっていない意図を、動く仕組みにする
<sub>"困った" を自作ツールで解く内製エンジニア。暮らしも、現場も。Front-end craft → Full-stack × AI</sub>
...
[![Role](https://img.shields.io/badge/意図_→_仕組み-0d1117?style=flat-square&logoColor=7d8590)](#)
...
>
> 手を動かす時間は長いけれど、いちばん価値が出ているのは制作そのものより **「変換」** のほうでした。
> ふわっとした「こうしたい」を、目的 → 設計 → タスク → 動いている状態、まで運ぶ仕事です。
...
```text
「こうしたい」          まだ言葉になっていない、経営や現場の意図
      │
      ↓ 翻訳          何のためか・何をもって良しとするかに置き換える
目的と判断基準
      │
      ↓ 構造化        Web／業務フロー／システム／コンテンツの形にする
設計
      │
      ↓ 分解          人・AI・ツールが実行できる大きさに割る
タスク
      │
      ↓ 実装          公開・運用・検証まで持っていく
動いている状態
...
<sub>だからWebサイトも「制作物」ではなく **事業のインフラ** として設計します。見た目より、その後2年運用できるかどうか。</sub>
...
- **実際に使われるものを優先する** — 動かないきれいな設計より、使われる70点
- **100%を待たない** — 70%で出して、動かしながら直す。完璧待ちの停滞がいちばん高くつく
- **見栄えではなく運用可能性で評価する** — 引き継げるか、2年後も回るか
- **情報は見える場所に置く** — 水面下のやりとりに閉じない
- **ボールの所在をはっきりさせる** — 誰がいつ判断するのかが決まっていない状態を放置しない
- **意味のない工程を減らす** — 増やすより、削るほうが効くことが多い
- **一般論ではなく、目の前の制約から決める** — 今の現場・今のコード・今の予算
- **高価な完成品より、費用対効果の高い自作** — ただし自作が目的にならないように

**AIの扱いについて**

```text
AIは目的ではない          道具として、成果が出る形に置く
AIに責任はない            承認・公開・運用の責任は人が持つ
AIはセキュリティを保証しない  前提として設計側で担保する
最終判断は人間             出力をそのまま通さない
```

<sub>Claude Code / Codex / ローカルLLM は「万能の1体」ではなく、**役割分担する作業者たち** として扱っています。</sub>
...
<sub>最終的に作りたいのは、Webサイトでも AI システムでもなく、<br/>**自分が全部を処理しなくても、意図したとおりに事業が進んでいく仕組み** です。</sub>
